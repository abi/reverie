...
function SimpleApp (opts, cb) {
  ...
  /**
   * Prefix used to namespace all user entries in LevelDB
   * @type {string}
   */
  self.userPrefix = 'user~'
   ...
   * @type {Object}
   */
  self.db = levelup(self.dbPath, { valueEncoding: 'json' })
...
SimpleApp.prototype.start = function (cb) {
  ...
  passport.deserializeUser(function (username, done) {
    self.db.get(self.userPrefix + username, function (err, user) {
      if (err) {
        done(err)
      } else {
        done(null, user)
  ...
  app.post('/register', function (req, res, next) {
    ...
    self.db.get(self.userPrefix + key, function (err) {
      if (err && err.name === 'NotFoundError') {
        // Hash the password and store it
        bcrypt.hash(value.password, 8, function (err, hash) {
          value.password = hash
          delete value._csrf
          self.db.put(self.userPrefix + key, value, function (err) {
            if (err) {
